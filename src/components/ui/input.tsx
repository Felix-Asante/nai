...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    { className, type, disabled = false, endContent, startContent, ...props },
    ref
  ) => {
    return (
      <div
      ...
      >
        ...
        <input
          type={type}
          className={cn(
            "w-full outline-none disabled:cursor-not-allowed disabled:bg-transparent",
            className
          )}
          ref={ref}
          {...props}
        ...
        />
        {endContent && <div className="ml-3">{endContent}</div>}
      </div>
    );
  }
);
